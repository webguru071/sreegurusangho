...
@section('navBreadCrumb')
    <nav aria-label="breadcrumb" class="ms-3">
        <ol class="breadcrumb m-1 mb-2">
            <li class="breadcrumb-item active" aria-current="page">Council member</li>
        </ol>
    </nav>
@endsection
...
@section('content')
    <div class="card">
        ...
        <div class="card-body p-3">
            <div class="mb-3">
                <a class="btn btn-primary" href="{{ route("council.member.add") }}" role="button">Add</a>
            </div>
            <div class="table-responsive">
                <table class="table table-sm table-bordered">
                    ...
                    <tbody>
                        @forelse ($councilMembers as $councilMemberIndex => $councilMember )
                            <tr>
                                <td>{{ $councilMemberIndex+1 }}</td>
                                <td>{{ $councilMember->name_en }} ({{ $councilMember->name_bn }})</td>
                                <td>{{ $councilMember->membership_type }} ({{ $councilMember->member_position }})</td>
                                <td>
                                    <a href="{{ route("council.member.edit",["id"=>$councilMember->id]) }}" class="btn btn-sm btn-primary"> Edit</a>
                                        ...
                                        <div class="modal-dialog modal-lg">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <p>
                                                        <ul>
                                                            <li>Council member will be delete.</li>
                                                        </ul>
                                                    </p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-sm btn-danger" data-bs-dismiss="modal">Close</button>
                                                    <form action="{{ route("council.member.delete",["id" => $councilMember->id]) }}" method="POST">
                                                        @csrf
                                                        @method("DELETE")
